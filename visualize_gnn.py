...
# Load labels
with open(os.path.join(data_folder, 'labels_1500.txt'), 'r') as f:
    labels = f.read().splitlines()

# Load shuffled document sentences
with open(os.path.join(data_folder, 'shuffle_1500.txt'), 'r') as f:
    sentences = f.read().splitlines()

# Load vocabulary
with open(os.path.join(data_folder, 'vocab_1500.txt'), 'r') as f:
...
# Define colors and sizes for documents and words
node_colors = []
node_sizes = []
for node in G.nodes():
    if G.nodes[node]['type'] == 'document':
        ...
        node_colors.append('lightgreen')  # Words in light green
