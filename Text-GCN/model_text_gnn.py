...
class TextGNN(nn.Module):
    ...
    def forward(self, pyg_graph, dataset):
        ...
        for i, layer in enumerate(self.layers):
            ...
            acts.append(outs)
        second_last_layer_embeddings = acts[-2]
        # print("second_last_layer_embeddings MIL GAYIII ", second_last_layer_embeddings)
        # np.savetxt('second_last_layer_embeddings.txt', second_last_layer_embeddings.detach().cpu().numpy(), delimiter=',')
        with open('second_last_layer_embeddings.txt', 'w') as f:
            np.savetxt(f, second_last_layer_embeddings.detach().cpu().numpy(), delimiter=',')
        print("Saved second last layer embeddings for the last epoch, overwriting previous ones.")
        
        print("SECOND LAST EMBEDDING KA SHAPE HAI ", second_last_layer_embeddings.shape)
        print("LAYERS KA SHAPE HAI ", len(self.layers))
        # print("I AM HOPING THESE ARE NODES IDS ",self.docs.keys())
